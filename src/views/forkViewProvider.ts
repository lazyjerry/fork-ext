...
export class ForkViewProvider implements vscode.WebviewViewProvider, vscode.Disposable {
  ...
  async openInFork(): Promise<void> {
    ...
    if (!location) {
      ...
      void vscode.window.showWarningMessage(`fooook：這裡不是 git 儲存庫（${where}）`);
      return;
    }
    ...
    switch (availability.kind) {
      ...
      case 'unsupportedPlatform':
        void vscode.window.showWarningMessage(
          `fooook：Fork 只提供 macOS 與 Windows 版本，目前平台是 ${availability.platform}。`,
        );
        return;
    }
  }
  ...
  private async handleMessage(message: ClientMessage): Promise<void> {
    switch (message.type) {
      ...
      case 'copyText':
        ...
        void vscode.window.setStatusBarMessage(`fooook：已複製${message.label}`, 2000);
        return;
    }
  }
  ...
  private async launchFork(cliPath: string, repoRoot: string): Promise<void> {
    try {
      ...
      void vscode.window.showErrorMessage(`fooook：執行 fork 指令失敗（${detail}）`);
    }
  }
  ...
  private async promptCliInstall(appPath: string): Promise<void> {
    ...
    if (picked === '複製安裝指令' && hint.copyCommand) {
      ...
      void vscode.window.showInformationMessage('fooook：安裝指令已複製，貼到終端機執行即可。');
      return;
    }
    if (picked === '開啟 Fork') {
